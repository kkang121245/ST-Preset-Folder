...
import {getSanitizedFilename, equalsIgnoreCaseAndAccents} from "/scripts/utils.js";
...
function syncFolderFilterToSelectedPreset() {
  ...
  const availableValues = new Set(Array.from(folderFilter.options).map((option) => option.value));
  const resolvedFolderId = availableValues.has(targetFolderId) ? targetFolderId : ALL_FOLDER_ID;
...
function setAssignedFolderId(presetName, folderId) {
  const state = getState();
  const isValidFolder = state.folders.some((folder) => folder.id === folderId);
...
function moveFolderByIndex(index, offset) {
  ...
  if (index < 0 || targetIndex < 0 || index >= state.folders.length || targetIndex >= state.folders.length) {
    return false;
  }
...
function ensureFolderManageButton() {
  ...
  const bindLabel = document.querySelector('label[for="bind_preset_to_connection"]');
...
function populateFolderFilterSelect(selectElement) {
  ...
  if (document.activeElement !== selectElement) {
    const currentOptions = Array.from(selectElement.options).map((o) => ({
      value: o.value,
      label: o.textContent,
    }));
    const isSame = currentOptions.length === options.length && options.every((opt, i) => currentOptions[i].value === opt.value && currentOptions[i].label === opt.label);
...
function applyPresetFilter(triggerChangeIfNeeded) {
  ...
  for (const option of Array.from(select.options)) {
    ...
    const assignedFolderId = state.assignments[presetName] || "";
    const isAssignedFolderValid = assignedFolderId && existingFolderIds.has(assignedFolderId);
    const normalizedAssignedFolderId = isAssignedFolderValid ? assignedFolderId : "";
...
function attachRenameInterceptor() {
  ...
  document.addEventListener(
    ...
    (event) => {
      ...
      const renameButton = target.closest('[data-preset-manager-rename="openai"]');
...
function queueAutoAssignForNewPresets() {
  ...
  const addedPresetNames = currentPresetNames.filter((name) => !knownPresetNames.has(name));
...
async function openRenamePresetWithFolderPopup() {
  ...
  populateFolderSelect(folderSelect, currentFolderId || UNCATEGORIZED_FOLDER_ID, true);
  ...
  const requestedName = await getSanitizedFilename(String(nameInput.value || "").trim());
  if (!requestedName) {
...
async function openFolderManagerPopup() {
  ...
  function refreshFolderControls(selectedFolderId = "") {
    ...
    if (state.folders.length > 0) {
      ...
      folderList.value = selectedFolderId && state.folders.some((folder) => folder.id === selectedFolderId) ? selectedFolderId : fallbackId;
    }
  ...
  function refreshPresetList() {
    ...
    const scopedPresetNames =
      previewFolderId ?
        allPresetNames.filter((name) => {
          const assignedFolderId = getAssignedFolderId(name) || UNCATEGORIZED_FOLDER_ID;
    ...
    if (!presetNames.length) {
      ...
      empty.textContent = keyword ? "검색 결과가 없습니다." : "표시할 프리셋이 없습니다.";
    ...
    for (const presetName of presetNames) {
      ...
      populateFolderSelect(folderSelect, getAssignedFolderId(presetName) || UNCATEGORIZED_FOLDER_ID, true);
  ...
  content.querySelector(".stpf-add-folder").addEventListener("click", async () => {
    const name = String(
    ...
    const state = getState();
    createFolder(name);
    refreshFolderControls(state.folders[state.folders.length - 1]?.id || "");
    refreshPresetList();
    ensureFolderFilterSelect();
    applyPresetFilter(false);
  });
  ...
  content.querySelector(".stpf-check-all").addEventListener("click", () => {
    const checkboxes = Array.from(content.querySelectorAll(".stpf-row-check"));
  ...
  content.querySelector(".stpf-rename-folder").addEventListener("click", async () => {
    ...
    const selectedFolder = getState().folders.find((folder) => folder.id === selectedFolderId);
  ...
  content.querySelector(".stpf-delete-folder").addEventListener("click", async () => {
    const selectedFolderId = folderList.value;
    if (!selectedFolderId) {
  ...
  content.querySelector(".stpf-apply-bulk").addEventListener("click", () => {
    ...
    if (folderId === ALL_FOLDER_ID) {
      toastr.info("전체 폴더는 보기 전용입니다. 적용할 폴더를 선택하세요.");
      return;
    }

    const checkedRows = Array.from(content.querySelectorAll(".stpf-row-check:checked"));
  ...
  content.querySelector(".stpf-preview-folder").addEventListener("click", () => {
    const selectedFolderId = bulkFolderSelect.value || "";
    previewFolderId = selectedFolderId === ALL_FOLDER_ID ? "" : selectedFolderId;
    refreshPresetList();
  });

  content.querySelector(".stpf-refresh-list").addEventListener("click", () => {
    refreshPresetList();
  });
  ...
  const closePopup = () => {
    ...
    document.querySelectorAll(".stpf-mini-overlay").forEach((node) => node.remove());
    content.remove();
  };
  ...
  content.querySelector(".stpf-close-popup").addEventListener("click", closePopup);
...
function showStpfInputCard({title, message, defaultValue = "", confirmText = "저장", cancelText = "취소"}) {
  return new Promise((resolve) => {
    ...
    cancelBtn.addEventListener("click", () => finish(null));
    confirmBtn.addEventListener("click", () => finish(String(input.value || "").trim()));
...
function showStpfConfirmCard({title, message, confirmText = "삭제", cancelText = "취소"}) {
  return new Promise((resolve) => {
    const overlay = document.createElement("div");
    overlay.className = "stpf-mini-overlay";
    overlay.innerHTML = `
...
function populateFolderSelect(selectElement, selectedValue, includeUncategorized, includeAllTop = false) {
  const state = getState();
  selectElement.innerHTML = "";
  ...
  const values = new Set(Array.from(selectElement.options).map((option) => option.value));
...
async function waitForOpenAIPresetUI() {
  ...
  while (Date.now() - start < timeoutMs) {
    if (getOpenAIPresetSelect() && document.querySelector('[data-preset-manager-rename="openai"]')) {
      return;
    }
